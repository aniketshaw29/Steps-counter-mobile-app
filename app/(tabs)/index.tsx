...
import { getPersonalBest, checkAndUpdatePersonalBest, PersonalBest } from '../../src/db/personalBest';
...
import { todayString } from '../../src/utils/dateHelpers';
...
export default function TodayScreen() {
  ...
  const [personalBest, setPersonalBest] = useState<PersonalBest>({ steps: 0, date: '' });
  ...
  // Check personal best whenever steps change
  useEffect(() => {
    if (steps > 0) {
      checkAndUpdatePersonalBest(steps, todayString()).then((newPB) => {
        if (newPB) {
          setPersonalBest({ steps, date: todayString() });
          setIsNewPB(true);
          setTimeout(() => setIsNewPB(false), 3000);
        }
      });
    }
  }, [steps]);
  ...
  return (
    <>
      ...
      >
        ...
        {personalBest.steps > 0 && (
          <View style={[styles.pbBanner, { backgroundColor: isNewPB ? C.primary : C.surfaceVariant }]}>
            <Text style={[styles.pbText, { color: isNewPB ? C.onPrimary : C.onSurfaceVariant }]}>
              {isNewPB ? '🌟 New personal best!' : `🏅 Best: ${personalBest.steps.toLocaleString()} steps`}
            </Text>
            {!isNewPB && personalBest.date ? (
              <Text style={[styles.pbDate, { color: C.onSurfaceVariant }]}>{personalBest.date}</Text>
            ) : null}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  pbText: { fontSize: FontSize.sm, fontWeight: '600' },
  pbDate: { fontSize: FontSize.xs },
